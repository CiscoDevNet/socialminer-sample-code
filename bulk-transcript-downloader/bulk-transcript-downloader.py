...
# CONSTANTS
SEARCH_API_URL = "https://{}/ccp-webapp/ccp/search/contacts?q=sc.sourceType:chat%20AND%20sc.socialContactStatus:handled&count={}&startIndex={}"
...
def main():
    ...
    search_response = make_search_request(SEARCH_API_URL.format(host,str(count),str(startIndex)), username, password)
    ...
    if transcript_count > 0:
        ...
        while(root[2].tag.find("link")!=-1):
            startIndex += count
            print("\nStarting at Index %d" % startIndex)
            search_response = make_search_request(SEARCH_API_URL.format(host, str(count), str(startIndex)), username,password)
            root = ElementTree.fromstring(search_response)
            transcript_count = len(root.findall('.//ChatTranscript'))
            print("\nFound %d chat transcripts. Starting export ...\n" % transcript_count)
        ...
        archive_name = TRANSCRIPT_ARCHIVENAME.format(host, time.strftime(FILENAME_TIMESTAMP_FORMAT,time.localtime(time.time())))
